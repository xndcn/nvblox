...
template <>
void initializeBlocksAsync<FloatVoxelBlock>(
    host_vector<FloatVoxelBlock*>& blocks, const CudaStream& cuda_stream,
    const MemoryType memory_type) {
  for (FloatVoxelBlock* ptr : blocks) {
    FloatVoxelBlock::initAsync(ptr, memory_type, cuda_stream);
  }
}
